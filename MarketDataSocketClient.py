...
class MDSocket_io(socketio.Client):
    ...
    def connect(self, headers={}, transports='websocket', namespaces=None, socketio_path='/apimarketdata/socket.io',
        ...
        self.thread = threading.Thread(target=self.sid.connect , args=(url, headers, transports, namespaces, socketio_path))
        self.thread.start()
    ...
    def on_message1505_json_partial(self, data):
        ...
        now = datetime.now()
        today = now.strftime("%H:%M:%S")
        print(today, 'in main 1505 partial, Instrument Property Change Event!' + data + ' \n')

        print('I received a 1505 Instrument Property Change Event!' + data)
